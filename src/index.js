...
const NAV_I18N = {
  ar: { students: "الطلاب", today: "حضور اليوم", print: "🖨️ كشف ورقي", promos: "📢 العروض", brand: "هارف · تسجيل الحضور" },
  en: { students: "Students", today: "Today's Attendance", print: "🖨️ Print Sheet", promos: "📢 Promotions", brand: "Harv · Attendance" }
};
...
${nav ? `<nav><a href="/admin${lang === "en" ? "?lang=en" : ""}">${n.students}</a><a href="/admin/today${lang === "en" ? "?lang=en" : ""}">${n.today}</a><a href="/admin/print">${n.print}</a><a href="/admin/promotions${lang === "en" ? "?lang=en" : ""}">${n.promos}</a></nav>` : ""}
<h1>${title}</h1>
${body}
</div>
</body></html>`;
  ...
  async fetch(request, env) {
    ...
    const PROCESS_I18N = {
      ar: {
        ...
        name: "اسم الطالب", school: "اسم المدرسة", stage: "الصف الدراسي",
      ...
      en: {
        ...
        name: "Student name", school: "School name", stage: "Grade",
    ...
    if (processMatch && request.method === "GET") {
      ...
      const student = await env.DB.prepare(
        "SELECT id, name, class, school, stage, phone, email, subjects, photo, photo_type, parent_phone FROM students WHERE id = ?"
    ...
    if (processMatch && request.method === "POST") {
      ...
      await env.DB.prepare(
        `UPDATE students SET name = ?, school = ?, stage = ?, class = ?, phone = ?, email = ?, subjects = ?, payment_method = ?, parent_phone = ?, status = 'approved' WHERE id = ?`
      ).bind(name, school, stage, stage, phone, email, subjects, paymentMethod, parentPhone, processMatch[1]).run();
...
<p class="no-print"><a href="/admin${langQs}">${t.back}</a></p>`;
    ...
    if (url.pathname === "/register" && request.method === "POST") {
      ...
      await env.DB.prepare(
        "INSERT INTO students (name, class, school, stage, phone, email, subjects, parent_phone, photo, photo_type, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')"
      ).bind(name, stage, school, stage, phone, email, subjects, parentPhone, photoBuf, photoType).run();
      return new Response(page(t.thanksTitle, `<div class="confirm"><strong>${t.thanks}</strong>${t.thanksBody}</div>`, { nav: false, lang }), { headers: { "content-type": "text/html;charset=utf-8" } });
    }
