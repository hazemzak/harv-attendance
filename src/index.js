...
const NAV_I18N = {
  ar: { students: "الطلاب", today: "حضور اليوم", print: "🖨️ كشف ورقي", brand: "هارف · تسجيل الحضور" },
  en: { students: "Students", today: "Today's Attendance", print: "🖨️ Print Sheet", brand: "Harv · Attendance" }
};
...
${nav ? `<nav><a href="/admin${lang === "en" ? "?lang=en" : ""}">${n.students}</a><a href="/admin/today${lang === "en" ? "?lang=en" : ""}">${n.today}</a><a href="/admin/print">${n.print}</a></nav>` : ""}
<h1>${title}</h1>
${body}
</div>
</body></html>`;
  ...
  async fetch(request, env) {
    ...
    const ADMIN_I18N = {
      ar: {
        ...
        pending: "بانتظار المراجعة", approve: "قبول", reject: "حذف",
        addName: "اسم الطالب", addNamePh: "اكتب اسم الطالب",
        addClass: "الصف", addClassPh: "مثال: ثانوية عامة", addSubmit: "إضافة طالب",
        empty: "لا يوجد طلاب مسجلين بعد."
      },
      en: {
        title: "Students", regLink: "Student registration link (share on WhatsApp):",
        pending: "Pending review", approve: "Approve", reject: "Delete",
        addName: "Student name", addNamePh: "Enter student name",
        addClass: "Class", addClassPh: "e.g. General Secondary", addSubmit: "Add student",
    ...
    if (url.pathname === "/admin" && request.method === "GET") {
      ...
      const { results } = await env.DB.prepare(
        "SELECT id, name, class, school, stage, phone, email, photo, photo_type, status FROM students ORDER BY (status='pending') DESC, name"
      ).all();
        ...
        return `<div class="card pending-card">
          ...
          <div>
            ...
            <div class="pending-actions">
              <form method="POST" action="/admin/students/${s.id}/approve${langQs}"><button type="submit">${t.approve}</button></form>
      ...
      const cards = approved.map((s, i) => {
        ...
        return `<div class="card ${i % 2 === 0 ? "stripe-a" : "stripe-b"}">${qrSvg(scanUrl)}<div><strong>${s.name}</strong><br><small>${s.class || ""}</small></div></div>`;
    ...
    const approveMatch = url.pathname.match(/^\/admin\/students\/(\d+)\/approve$/);
    if (approveMatch && request.method === "POST") {
      const lang = langOf(url);
      await env.DB.prepare("UPDATE students SET status = 'approved' WHERE id = ? AND status = 'pending'").bind(approveMatch[1]).run();
      return Response.redirect(url.origin + "/admin" + (lang === "en" ? "?lang=en" : ""), 303);
    }
    ...
    const REGISTER_I18N = {
      ar: {
        ...
        stage: "الصف الدراسي", stagePh: "اختر الصف", stage2: "تانية ثانوي", stage3: "تالتة ثانوي",
      ...
      en: {
        ...
        stage: "Grade", stagePh: "Choose grade", stage2: "2nd Secondary", stage3: "3rd Secondary",
    ...
    if (url.pathname === "/register" && request.method === "GET") {
      ...
      const body = `<form method="POST" action="/register${langQs}" enctype="multipart/form-data">
        ...
        <select name="stage" required>
          ...
          <option value="تانية ثانوي">${t.stage2}</option>
          <option value="تالتة ثانوي">${t.stage3}</option>
        </select>
        <label>${t.phone}</label>
    ...
    if (url.pathname === "/register" && request.method === "POST") {
      ...
      await env.DB.prepare(
        "INSERT INTO students (name, class, school, stage, phone, email, photo, photo_type, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')"
      ).bind(name, stage, school, stage, phone, email, photoBuf, photoType).run();
      return new Response(page(t.thanksTitle, `<div class="confirm"><strong>${t.thanks}</strong>${t.thanksBody}</div>`, { nav: false, lang }), { headers: { "content-type": "text/html;charset=utf-8" } });
    }
    ...
    if (url.pathname === "/admin/today" && request.method === "GET") {
      ...
      const rows = results.map((r, i) => `<div class="card ${i % 2 === 0 ? "stripe-a" : "stripe-b"}"><div><strong>${r.name}</strong><br><small>${r.scanned_at}</small></div></div>`).join("") || `<p class="empty">لا يوجد تسجيل حضور اليوم.</p>`;
