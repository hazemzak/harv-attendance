...
import CAIRO_REGULAR_TTF from "../assets/fonts/Cairo-Regular.ttf";
import CAIRO_BOLD_TTF from "../assets/fonts/Cairo-Bold.ttf";
...
// Lazily initializes resvg-wasm once per Worker isolate (real cost, paid
// once at cold start) rather than on every request -- the module-scope
// promise is the standard idiom for expensive one-time init that survives
...
async function svgToPng(svg) {
  ...
  const resvg = new Resvg(svg, {
    font: {
      fontBuffers: [new Uint8Array(CAIRO_REGULAR_TTF), new Uint8Array(CAIRO_BOLD_TTF)],
...
// The branded student ID card (Hazem: "the ID card should have their QR
// code, and the teachers they signed with... in form of an image"). Built
// as SVG text first, then rasterized to PNG by svgToPng() above -- resvg is
// the only way to get a real raster image out of a Cloudflare Worker
// (no canvas/DOM/Puppeteer available in this runtime).
// Deliberately curated fields only (name, subjects+teachers, QR) -- never
// phone/address/balance, even though the full record has them (mirrors the
// existing /student page's same restraint, and the physical print-ticket).
function studentCardSvg(student, bookings, scanUrl) {
  const W = 900, H = 1300;
  const RED = "#D42027", INK = "#1A2744", PAPER = "#FAFAF8", LINE = "#E4E1DA";
  const rowsStartY = 500, rowGap = 46;
  const rows = bookings.map(b => `${subjectLabelFor(b.subject)} — ${b.teacher_name || "—"}`);
  const rowsSvg = rows.length
    ? rows.map((line, i) =>
        `<text x="${W - 60}" y="${rowsStartY + i * rowGap}" text-anchor="end" font-family="Cairo" font-weight="400" font-size="26" fill="${INK}">${escapeHtml(line)}</text>`
      ).join("")
    : `<text x="${W - 60}" y="${rowsStartY}" text-anchor="end" font-family="Cairo" font-weight="400" font-size="24" fill="#5A6784">لسه مفيش مواد مسجلة</text>`;
  const qrSize = 420;
  const qrX = (W - qrSize) / 2;
  const qrY = rowsStartY + Math.max(rows.length, 1) * rowGap + 40;
  // Own module-to-path math, not qrSvg()'s output -- that helper returns a
  // standalone <svg> sized to its own native cellSize*modules px, which
  // wouldn't fill/match this card's fixed qrSize box if just nested as-is.
  // Same qrcode-generator call qrSvg() itself makes, just laid out to an
  // exact target size here instead.
  const qr = qrcode(0, "M");
  qr.addData(scanUrl);
  qr.make();
  ...
  for (let r = 0; r < qrModules; r++) {
    for (let c = 0; c < qrModules; c++) {
      if (qr.isDark(r, c)) qrPath += `M${(c * cell).toFixed(2)},${(r * cell).toFixed(2)}h${cell.toFixed(2)}v${cell.toFixed(2)}h${-cell.toFixed(2)}z `;
    }
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
    <rect width="${W}" height="${H}" fill="${PAPER}"/>
    <rect width="${W}" height="190" fill="${RED}"/>
    ${LOGO_B64 ? `<image x="60" y="55" width="80" height="80" href="data:image/png;base64,${LOGO_B64}"/>` : ""}
    <text x="${W - 60}" y="90" text-anchor="end" font-family="Cairo" font-weight="700" font-size="34" fill="${PAPER}">هارف</text>
    <text x="${W - 60}" y="130" text-anchor="end" font-family="Cairo" font-weight="400" font-size="24" fill="${PAPER}">بطاقة الطالب</text>
    <text x="${W - 60}" y="290" text-anchor="end" font-family="Cairo" font-weight="700" font-size="46" fill="${INK}">${escapeHtml(student.name)}</text>
    ${student.class ? `<text x="${W - 60}" y="335" text-anchor="end" font-family="Cairo" font-weight="400" font-size="26" fill="#5A6784">${escapeHtml(student.class)}</text>` : ""}
    <line x1="60" y1="380" x2="${W - 60}" y2="380" stroke="${LINE}" stroke-width="2"/>
    <text x="${W - 60}" y="440" text-anchor="end" font-family="Cairo" font-weight="700" font-size="28" fill="${INK}">المواد المسجلة</text>
    ${rowsSvg}
    <rect x="${qrX - 30}" y="${qrY - 30}" width="${qrSize + 60}" height="${qrSize + 60}" rx="20" fill="#fff" stroke="${LINE}" stroke-width="2"/>
    <path transform="translate(${qrX},${qrY})" d="${qrPath}" fill="${INK}"/>
    <text x="${W / 2}" y="${qrY + qrSize + 55}" text-anchor="middle" font-family="Cairo" font-weight="400" font-size="22" fill="#5A6784">اعرض الكود ده للاستقبال عند دخولك</text>
    <rect x="0" y="${H - 70}" width="${W}" height="70" fill="${INK}"/>
    <text x="${W / 2}" y="${H - 28}" text-anchor="middle" font-family="Cairo" font-weight="400" font-size="22" fill="${PAPER}">واتساب 01064949395 · harvcentereg.com</text>
  </svg>`;
}
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/admin" && request.method === "GET") {
      ...
      const cards = approved.map((s, i) => {
        ...
        const studentCardUrl = `${url.origin}/card/${s.id}.png`;
        const waHref = s.phone ? waLink(s.phone, `أهلاً ${s.name} 👋 دي بطاقتك في هارف، احفظها وافتحها كل يوم علشان تدخل بيها: ${studentCardUrl}`) : null;
    ...
    const SUCCESS_I18N = {
      ar: {
        ...
        wa: "🪪 ابعت البطاقة للطالب", waParent: "👪 ابعت الرابط لولي الأمر", print: "🖨️ اطبع تذكرة الدخول",
        ticketHint: "الطالب يستخدم الكود ده كل يوم للدخول — المدرسة تديه ورقة مطبوعة و/أو بطاقته على واتساب.",
        waText: (name, link) => `أهلاً بيك في هارف يا ${name}! دي بطاقتك، احفظها وافتحها كل يوم علشان تدخل بيها: ${link}`,
      ...
      en: {
        ...
        wa: "🪪 Send card to student", waParent: "👪 Send link to parent", print: "🖨️ Print entry ticket",
        ticketHint: "The student uses this code every day to enter — hand them the printed ticket and/or their card on WhatsApp.",
        waText: (name, link) => `Welcome to Harv, ${name}! Here's your card, save it and open it daily to check in: ${link}`,
    ...
    if (successMatch && request.method === "GET") {
      ...
      // Two different links, two different audiences (deliberately not
      // unified): the student gets the card image (no platform access at
      // all -- Hazem's ask), the parent keeps the live /student page since
      // that's the one place they can check "did they attend today or not"
      // on demand, a real anti-fraud feature that a static image can't do.
      const studentCardUrl = `${url.origin}/card/${student.id}.png`;
      const studentPageUrl = `${url.origin}/student?id=${student.id}`;
      const waHref = student.phone ? waLink(student.phone, t.waText(student.name, studentCardUrl)) : null;
...
<div class="reg-link no-print">${t.linkLabel}<br><a href="${studentCardUrl}">${studentCardUrl}</a></div>
<p class="no-print"><a href="/admin${langQs}">${t.back}</a></p>`;
    ...
    if (estamaraMatch && request.method === "GET") {
      ...
      // Same student-card vs. parent-tracking-link split as the success
      // page above (see its comment) -- the parent's link stays live-status,
      // the student's becomes the static card image.
      const studentCardUrl = `${url.origin}/card/${student.id}.png`;
      const studentPageUrl = `${url.origin}/student?id=${student.id}`;
      const stU = SUCCESS_I18N[lang];
      const waStudentHref = student.phone ? waLink(student.phone, stU.waText(student.name, studentCardUrl)) : null;
      const hasSeparateParent = student.parent_phone && student.parent_phone !== student.phone;
      ...
      <div class="reg-link no-print">${lang === "en" ? "Student's card link:" : "رابط بطاقة الطالب:"}<br><a href="${studentCardUrl}">${studentCardUrl}</a></div>`;
      const body = `
...
(function(){
  ...
  openHash();
    ...
    // Public by design, same as /register and /student -- this is the link
    // sent to the STUDENT directly (not the parent's tracking link, which
    // stays pointed at /student since it needs live "attended today or not"
    // status a static image can never provide). Hazem: "I don't want them to
    // receive a link, I just want them to receive their QR code in a
    // picture... so they never have access to our platform." Opening this
    // link loads the image itself -- no HTML page, no nav, nothing else on
    // the platform is reachable from it. Deliberately curated fields only
    // (name, class, subjects+teachers, QR) -- never phone/address/balance,
    // matching the same restraint /student and the print-ticket already use.
    const cardMatch = url.pathname.match(/^\/card\/(\d+)\.png$/);
    if (cardMatch && request.method === "GET") {
      const student = await env.DB.prepare("SELECT id, name, class, status FROM students WHERE id = ?").bind(cardMatch[1]).first();
      if (!student || student.status !== "approved") return new Response("", { status: 404 });
      const bookings = await getBookings(env, student.id);
      const scanUrl = `${url.origin}/scan?student=${student.id}`;
      const png = await svgToPng(studentCardSvg(student, bookings, scanUrl));
